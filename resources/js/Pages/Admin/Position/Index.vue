<template>
  <div class="flex h-screen">
    <AdminLayout>
      <template #header>
        <h1 class="text-2xl font-bold m-4 text-gray-600 dark:text-stroke">Positions</h1>
        <div class="flex justify-between m-4 mb-0">
          <Search />
          <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold px-2 rounded" @click="createPosition">
            Create New
          </button>
        </div>
      </template>
      <div class="rounded-lg border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5  xl:pb-1">
        <div class="max-w-full overflow-x-auto">
          <table class="w-full table-auto">
            ...
            <tbody>
              <tr v-for="position in positions" :key="position.id" class="hover:bg-gray-100 dark:text-white">
                <td class="py-5 px-4 pl-9 xl:pl-11 text-left">
                  <h5 class="font-medium text-black dark:text-white">{{ position.title }}</h5>
                </td>
                <td class="py-5 px-4">
                  <p class="inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium" :class="{ 'text-green-700': position.is_active, 'text-red-500': !position.is_active }">
